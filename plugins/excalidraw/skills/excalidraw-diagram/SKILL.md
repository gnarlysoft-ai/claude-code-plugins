...
# Excalidraw Diagram Creator — Gnarlysoft Edition

Generate `.excalidraw` JSON files that **argue visually**, not just display information. All diagrams use the **Gnarlysoft dark mode theme** — dark canvas (`#1a1a1a`) with purple accent colors.
...
- Always pair a vivid stroke with a dark/muted fill for contrast on the dark canvas
...
**Note:** The canvas background is `#1a1a1a` (Gnarlysoft dark mode) instead of the default white.
...
```bash
cd <plugin-path>/skills/excalidraw-diagram/references && uv run python render_excalidraw.py <path-to-file.excalidraw>
```
...
- **Dark mode specific**: Text not readable against the dark canvas (use light colors from palette)
...
```bash
cd <plugin-path>/skills/excalidraw-diagram/references
uv sync
...
### Dark Mode Specific
21. **Canvas background**: `#1a1a1a` set in appState
22. **Text readability**: All text uses light colors from palette (no dark text on dark canvas)
23. **Fill contrast**: Shape fills are muted/dark, strokes are vivid for visibility
24. **Evidence artifacts**: Code blocks use `#1a1a1a` background with colored text
